...
from data_preprocessing.lingmess_data_processor import LingMess_Dataset, LingMess_Dataloader
from evaluators.metrics import muc, b_cubed, ceafe, f1
from evaluators.evaluators import CorefEvaluator, MentionEvaluator


class TrainingModule_lingmess_no_distillation(pl.LightningModule):
    def __init__(
        self,
        ...
        model_cfg, # model_cfg={"llm_pretrained_name":str, "max_span_len":int, "top_lambda":float, "ffnn_size_mention_detector":int, "ffnn_size_mention_linker":int, "dropout_prob":float}
    ...
    def get_transformed_dataset(self, dataset_raw: Dataset):
        ...
        dataset_train=LingMess_Dataset(dataset_raw["train"], tokenizer, self.preprocessing_cfg["tokenizer_type"], self.preprocessing_cfg["max_seq_len"])
        dataset_val=LingMess_Dataset(dataset_raw["validation"], tokenizer, self.preprocessing_cfg["tokenizer_type"], self.preprocessing_cfg["max_seq_len"])
        dataset_test=LingMess_Dataset(dataset_raw["test"], tokenizer, self.preprocessing_cfg["tokenizer_type"], self.preprocessing_cfg["max_seq_len"])    
    ...
    def train_dataloader(self) -> TRAIN_DATALOADERS:
        return LingMess_Dataloader(self.dataset_train, num_workers=self.preprocessing_cfg["num_workers"], max_total_seq_len=self.preprocessing_cfg["max_total_seq_len"], is_training=True)
    
    def val_dataloader(self) -> EVAL_DATALOADERS:
        return LingMess_Dataloader(self.dataset_val, num_workers=self.preprocessing_cfg["num_workers"], max_total_seq_len=self.preprocessing_cfg["max_total_seq_len"], is_training=False)
    
    def test_dataloader(self) -> EVAL_DATALOADERS:
        return LingMess_Dataloader(self.dataset_test, num_workers=self.preprocessing_cfg["num_workers"], max_total_seq_len=self.preprocessing_cfg["max_total_seq_len"], is_training=False)
    
    #preparing training    
    def initialize_model(self):
        self.model=LingMess_model(self.model_cfg)
