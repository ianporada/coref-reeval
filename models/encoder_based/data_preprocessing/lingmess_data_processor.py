...
class LingMess_Dataset(Dataset):
    def __init__(self, dataset_raw: Dataset, tokenizer, tokenizer_type, max_seq_len): 
        ...
        for doc in self.dataset_raw:
            ...
            if self.tokenizer_type!="t5":
                ...
                subtoken_map_list=[] #subtoken_map_list(num_subtokens): record the token number of each subtoken, None for <cls> and <sep> tokens and speaker info tokens
                new_token_map_list=[] #new_token_map_list(num_tokens): record the word number of each token, None for speaker info token
                last_speaker=None
                token_idx=0 #the token idx considering the speaker info
                for idx, (word, speaker) in enumerate(zip(input_words_list, speakers_list)):
                    if last_speaker!=speaker:
                        ...
                        assert len(speaker.split())==1 #each speaker only contains 1 token
                        last_speaker=speaker
                        new_token_map_list+=[None, None, None]
                        token_idx+=3
                    else:
                        speaker_prefix=[]
                    for _ in range(len(speaker_prefix)):
                        end_token_idx_to_word_idx.append(idx)
                        subtoken_map_list.append(None)
            ...
            self.coref_examples.append((token_ids_list, new_clusters, input_words_list, subtoken_map_list, new_token_map_list))
    ...
    def pad_batch(self, batch, max_length):
        if self.tokenizer_type!="t5":
            ...
            for example in batch:
                ...
                tokens_per_example=example[2]+[None]*(max_length-len(example[2])-2)
                subtoken_map_per_example=[None]+example[3]+[None]+[None]*(max_length-len(example[3])-2)
                new_token_map_per_example=example[4]+[None]*(max_length-len(example[4])-2)
                ...
                tokens.append(tokens_per_example)
                subtoken_map.append(subtoken_map_per_example)
                new_token_map.append(new_token_map_per_example)
        else:
            max_length+=1
            input_ids=[]
            input_masks=[]
            ...
            for example in batch:
                ...
                gold_clusters_per_example=self.pad_clusters(example[1])
                tokens_per_example=example[2]+[None]*(max_length-len(example[2])-1)
                subtoken_map_per_example=example[3]+[None]+[None]*(max_length-len(example[3])-1)
                new_token_map_per_example=example[4]+[None]*(max_length-len(example[4])-1)
                input_ids.append(input_ids_per_example)
                input_masks.append(input_masks_per_example)
                gold_clusters.append(gold_clusters_per_example)
                tokens.append(tokens_per_example)
        ...
        return (input_ids, input_masks, gold_clusters, tokens, subtoken_map, new_token_map)
    
    def pad_batch_leftover(self, batch, max_length): #used for LLMs taking short context such as roberta
        input_ids, input_masks, gold_clusters, tokens, subtoken_map, new_token_map=self.pad_batch(batch, max_length) #pad to the longest doc in the batch
        ...
        if leftover_input_ids==None:
            return (input_ids.squeeze(1), input_masks.squeeze(1), gold_clusters, tokens, subtoken_map, new_token_map)
        else:
            ...
            return (input_ids, input_masks, gold_clusters, tokens, subtoken_map, new_token_map, leftover_input_ids, leftover_input_masks)
    

class LingMess_Dataloader(DataLoader):
...
class LingMess_Dataset_GAP(Dataset):  
    def __init__(self, dataset_raw: Dataset, tokenizer, tokenizer_type, max_seq_len): 
        ...
        for doc in self.dataset_raw:
            ...
            token_ids_list=[] #token_ids_list(num_subtokens): list of all subtokens in the doc
            subtoken_map_list=[] #subtoken_map_list(num_subtokens): record the token number of each subtoken, None for <cls> and <sep> tokens
            new_token_map_list=[] #new_token_map_list(num_tokens): record the word number of each token
            character_offset=0
            word_idx_to_start_token_idx=dict()
            word_idx_to_end_token_idx=dict()
            ...
            self.coref_examples.append((token_ids_list, input_words_list, subtoken_map_list, new_token_map_list, (doc_id, pronoun_subtoken_idx, A_subtoken_idx, B_subtoken_idx)))
    ...
    def pad_batch(self, batch, max_length):
        ...
        for example in batch:
            input_ids_per_example=[self.tokenizer.cls_token_id]+example[0]+[self.tokenizer.sep_token_id]+[self.tokenizer.pad_token_id]*(max_length-len(example[0])-2)
            input_masks_per_example=[1]*(len(example[0])+2)+[0]*(max_length-len(example[0])-2)
            tokens_per_example=example[1]+[None]*(max_length-len(example[2])-2)
            subtoken_map_per_example=[None]+example[2]+[None]+[None]*(max_length-len(example[3])-2)
            new_token_map_per_example=example[3]+[None]*(max_length-len(example[4])-2)
            input_ids.append(input_ids_per_example)
            input_masks.append(input_masks_per_example)
            tokens.append(tokens_per_example)
            subtoken_map.append(subtoken_map_per_example)
            new_token_map.append(new_token_map_per_example)
            GAP_gold_pairs_list.append(example[4])
                
        input_ids=torch.tensor(input_ids, dtype=torch.long)
        input_masks=torch.tensor(input_masks, dtype=torch.long)
        return (input_ids, input_masks, None, tokens, subtoken_map, new_token_map, GAP_gold_pairs_list) #GAP_gold_pairs_list should not be passed to the model 
    
    def pad_batch_leftover(self, batch, max_length): 
        return self.pad_batch(batch, max_length) 
    
    
class LingMess_Dataset_official(Dataset): #for official evaluation on ontonotes
    def __init__(self, dataset_raw: Dataset, tokenizer, max_seq_len): 
        ...
        for doc in self.dataset_raw:
            ...
            subtoken_map_list_2=[0] #subtoken_map_list_2(num_subtokens): list of word_id for each subtoken in the doc
            last_speaker=None
            token_idx=0 #the token idx considering the speaker info
            for idx, (word, speaker, sent_id) in enumerate(zip(input_words_list, speakers_list, sentence_id_list)):
                ...
                for _ in range(len(speaker_prefix)):
                    ...
                    subtoken_map_list_2.append(idx)
                token_ids_list.extend(speaker_prefix)
                word_idx_to_start_token_idx[idx]=len(token_ids_list)+1  # +1 for <s>
                tokenized=self.tokenizer.encode(" " + word, add_special_tokens=False)
                for _ in range(len(tokenized)):
            ...
            subtoken_map_list_2.append(subtoken_map_list_2[-1])
            
            self.coref_examples.append((token_ids_list, new_clusters, input_words_list, subtoken_map_list, new_token_map_list, (doc_id, hf_sentences, sentence_map_list, subtoken_map_list_2, global_word_idx_to_local_word_idx)))
    ...
    def pad_batch(self, batch, max_length):
        ...
        for example in batch:
            ...
            subtoken_map.append(subtoken_map_per_example)
            new_token_map.append(new_token_map_per_example)
            official_eval_info.append(example[5])
            
        input_ids=torch.tensor(input_ids, dtype=torch.long)
        input_masks=torch.tensor(input_masks, dtype=torch.long)
        gold_clusters=torch.tensor(gold_clusters, dtype=torch.long)
        return (input_ids, input_masks, gold_clusters, tokens, subtoken_map, new_token_map, official_eval_info)
    
    def pad_batch_leftover(self, batch, max_length): #used for LLMs taking short context such as roberta
        input_ids, input_masks, gold_clusters, tokens, subtoken_map, new_token_map, official_eval_info=self.pad_batch(batch, max_length) #pad to the longest doc in the batch
        input_ids, input_masks, gold_clusters=input_ids.tolist(), input_masks.tolist(), gold_clusters.tolist()
        ...
        gold_clusters=torch.tensor(gold_clusters, dtype=torch.long)
        if leftover_input_ids==None:
            return (input_ids.squeeze(1), input_masks.squeeze(1), gold_clusters, tokens, subtoken_map, new_token_map, official_eval_info)
        else:
            leftover_input_ids=torch.tensor(leftover_input_ids, dtype=torch.long)
            leftover_input_masks=torch.tensor(leftover_input_masks, dtype=torch.long)
            return (input_ids, input_masks, gold_clusters, tokens, subtoken_map, new_token_map, leftover_input_ids, leftover_input_masks, official_eval_info)