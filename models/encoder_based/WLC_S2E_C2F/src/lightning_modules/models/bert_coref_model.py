...
import sys
sys.path.append("src")
from utils import bucket_distance, batch_select
    
""" config: {"feature_emb_size":int, "dropout_rate":float, "bert_pretrained_name":str, "max_span_width":int, 
    ...
    Ex: {"feature_emb_size":64, "dropout_rate":0.1, "bert_pretrained_name":"bert-base-uncased", "max_span_width":30, 
    "max_segment_len":128, "use_span_width_to_compute_emb":True, "span_emb_compute_method":"attention", "ffnn_size":16, "ffnn_depth":1,
    "max_num_extracted_spans":100, "top_span_ratio":0.1, "crossing_mentions_allowed":False, "max_top_antecedents":28, 
    ...
    "num_genres":10, "max_num_segments":11}
    
    "use_span_width_to_compute_emb": whether to consider the span width when computing span embedding for each span
    "ffnn_size"、"ffnn_depth": the hidden size and depth for the MLP calculating mention score of each span and the MLP calculating fine coref score for each span pair 
    "max_num_extracted_spans": how many spans with highest mention scores are kept, "top_span_ratio": what percentage of spans with highest mention scores are kept
    "crossing_mentions_allowed": whether we can have overlapping corefered spans, it should be False for ontonotes-v5 dataset 
    "max_top_antecedents": how many antecedents of each top span are kept in coarse pruning
    "use_span_dist_to_compute_rough_score": whether to consider span distance when computing rough corefer scores for each span pair 
    "use_fine_score": whether to compute fine score as the final coref score
    "use_speaker_info_to_compute_fine_score": whether to use speaker info to compute fine score
    "use_genre_info_to_compute_fine_score": whether to use genre info to compute fine score
    "use_seg_dist_to_compute_fine_score": whether to use segment distance between each two spans to compute fine score
    "use_antecedent_dist_to_compute_fine_score": whether to use token distance between each two spans to compute fine score
    "max_num_segments": the maximum number of segments input to the model each time
"""

class BertCorefModel(nn.Module):
    def __init__(self, config, device=None):
        ...
        self.span_emb_attn=self.make_ffnn(self.bert_emb_size, 0, 1) if self.config["span_emb_compute_method"]=="attention" else None
    ...
    def get_input_emb(self, input_ids, input_mask, speaker_ids):
        token_emb,_=self.bert(input_ids, attention_mask=input_mask, return_dict=False) #token_emb(num_segments,sentence_max_len,emb_size)
