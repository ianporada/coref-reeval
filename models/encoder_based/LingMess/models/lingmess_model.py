...
from transformers import AutoModel, AutoConfig, BertPreTrainedModel, T5EncoderModel
...
"""config: {"llm_pretrained_name":str, "llm_type":str, "max_span_len":int, "top_lambda":float, "ffnn_size":int, "dropout_prob":float, "num_gpus":int}
"""
...
class LingMess_model(nn.Module):
    def __init__(self, config):
        ...
        if "num_gpus" in config:
            self.num_gpus=config["num_gpus"] #if needed to put different parts of models on different gpus so that the model can fit
        else: self.num_gpus=1
        if "handle_big_llm" in config:
            self.handle_big_llm=config["handle_big_llm"] #if needed to split llm on different gpus
        else: self.handle_big_llm=False
    ...
    def forward(self, input_ids, input_masks, gold_clusters, tokens, subtoken_map, new_token_map, leftover_input_ids=None, leftover_input_masks=None):
        ...
        """
        if self.num_gpus==2 and torch.cuda.device_count==2:
            input_ids, input_masks=input_ids.cuda(1), input_masks.cuda(1)
            if leftover_input_ids!=None:
                leftover_input_ids, leftover_input_masks=leftover_input_ids.cuda(1), leftover_input_masks.cuda(1)
            if self.handle_big_llm==False:
                self.llm_encoder.cuda(1)
            elif self.handle_big_llm==True and self.device_map!=None:
                dispatch_model(self.llm_encoder, device_map=self.device_map)
        ...
        if self.num_gpus==2:
            input_emb, input_masks=input_emb.cuda(0), input_masks.cuda(0)
        self.device=input_emb.device
