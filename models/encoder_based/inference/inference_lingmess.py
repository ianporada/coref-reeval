...
from training_modules.lingmess_model_no_distillation import TrainingModule_lingmess_no_distillation
from training_modules.s2e_model_no_distillation import TrainingModule_s2e_no_distillation
from data_preprocessing.lingmess_data_processor import LingMess_Dataset, LingMess_Dataloader, LingMess_Dataset_GAP, LingMess_Dataset_official
...
@hydra.main(config_path='../config', config_name='config_inference_lingmess')
def main(cfg : DictConfig) -> None:
    #load trained model
    training_module=TrainingModule_lingmess_no_distillation.load_from_checkpoint(cfg["model_ckpt"])
    ...
    if cfg["dataset_name"]=="ontonotes":
        if cfg["use_official_scorer"]==False:
            ...
            if cfg["dataset_split"]=="train":
                dataset=LingMess_Dataset(dataset_raw["train"], tokenizer, cfg["preprocessing_cfg"]["tokenizer_type"], cfg["preprocessing_cfg"]["max_seq_len"])
            elif cfg["dataset_split"]=="val":
                dataset=LingMess_Dataset(dataset_raw["validation"], tokenizer, cfg["preprocessing_cfg"]["tokenizer_type"], cfg["preprocessing_cfg"]["max_seq_len"])
            elif cfg["dataset_split"]=="test":
                dataset=LingMess_Dataset(dataset_raw["test"], tokenizer, cfg["preprocessing_cfg"]["tokenizer_type"], cfg["preprocessing_cfg"]["max_seq_len"])
            dataloader_singleBatch=LingMess_Dataloader(dataset, num_workers=1, max_total_seq_len=cfg["preprocessing_cfg"]["max_total_seq_len"], is_training=False)
            dataloader_maxBatch=LingMess_Dataloader(dataset, num_workers=1, max_total_seq_len=cfg["preprocessing_cfg"]["max_total_seq_len"], is_training=True)
        else:
            dataset_raw=load_dataset('conll2012_ontonotesv5', 'english_v4', cache_dir=cfg["cache_dir"])
            dataset_raw=get_doc_parts_official(dataset_raw)
            tokenizer=AutoTokenizer.from_pretrained(cfg["preprocessing_cfg"]["tokenizer_name"])
            if cfg["dataset_split"]=="train":
                dataset=LingMess_Dataset_official(dataset_raw["train"], tokenizer, cfg["preprocessing_cfg"]["max_seq_len"])
            elif cfg["dataset_split"]=="val":
                dataset=LingMess_Dataset_official(dataset_raw["validation"], tokenizer, cfg["preprocessing_cfg"]["max_seq_len"])
            elif cfg["dataset_split"]=="test":
                dataset=LingMess_Dataset_official(dataset_raw["test"], tokenizer, cfg["preprocessing_cfg"]["max_seq_len"])
            dataloader_singleBatch=LingMess_Dataloader(dataset, num_workers=1, max_total_seq_len=cfg["preprocessing_cfg"]["max_total_seq_len"], is_training=False)
            dataloader_maxBatch=LingMess_Dataloader(dataset, num_workers=1, max_total_seq_len=cfg["preprocessing_cfg"]["max_total_seq_len"], is_training=True)
        ...
        if cfg["use_official_scorer"]==False:
            ...
            tokenizer=AutoTokenizer.from_pretrained(cfg["preprocessing_cfg"]["tokenizer_name"])
            if cfg["dataset_split"]=="train":
                dataset=LingMess_Dataset_GAP(dataset_raw["train"], tokenizer, cfg["preprocessing_cfg"]["tokenizer_type"], cfg["preprocessing_cfg"]["max_seq_len"])
            elif cfg["dataset_split"]=="val":
                dataset=LingMess_Dataset_GAP(dataset_raw["validation"], tokenizer, cfg["preprocessing_cfg"]["tokenizer_type"], cfg["preprocessing_cfg"]["max_seq_len"])
            elif cfg["dataset_split"]=="test":
                dataset=LingMess_Dataset_GAP(dataset_raw["test"], tokenizer, cfg["preprocessing_cfg"]["tokenizer_type"], cfg["preprocessing_cfg"]["max_seq_len"])
            dataloader=LingMess_Dataloader(dataset, num_workers=1, max_total_seq_len=cfg["preprocessing_cfg"]["max_total_seq_len"], is_training=False)
    ...
    if (cfg["dataset_name"]=="ontonotes" or cfg["dataset_name"]=="ontoGUM") and cfg["use_official_scorer"]==False:
        ...
        with torch.no_grad():
            ...
            for batch in iter(tqdm(dataloader_singleBatch)):
                if cfg["inference_device"]=="gpu":
                    batch=[batch[i].cuda(0) for i in range(3)]+[batch[i] for i in range(3,6)]+[batch[i].cuda(0) for i in range (6,8) if len(batch)>=7]
    ...
    if cfg["dataset_name"]=="ontonotes" and cfg["use_official_scorer"]==True: 
        with torch.no_grad():
            ...
            for batch in iter(tqdm(dataloader_singleBatch)):
                ...
                if cfg["inference_device"]=="gpu":
                    batch=[batch[i].cuda(0) for i in range(3)]+[batch[i] for i in range(3,6)]+[batch[i].cuda(0) for i in range (6,8) if len(batch)>=8]
                device=batch[0].device
                assert len(batch[0])==1 #batch size should be 1 doc
                torch.cuda.reset_peak_memory_stats()
                torch.cuda.empty_cache()
    ...
    if cfg["dataset_name"]=="GAP":
        ...
        with torch.no_grad():
            for batch in iter(tqdm(dataloader)):
                ...
                if cfg["inference_device"]=="gpu":
                    batch=[batch[i].cuda(0) for i in range(2)]+[batch[i] for i in range(2,6)]
                assert len(batch[0])==1 #batch size should be 1 doc
                _, output_list=model(*batch)
